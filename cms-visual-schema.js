(function () {
  ...
  window.INFINITY_VISUAL_SCHEMA = Object.freeze([
    ...
    { key: 'portfolio', label: 'Selected Work', description: 'The three project categories shown on the homepage.', icon: '03', fields: [
      field('kicker', 'Section label', 'Selected Work', '精选作品'), field('titleLine1', 'Title — first line', 'Ideas, made', '将创意化为'), field('titleAccent', 'Highlighted title', 'tangible.', '真实成果。'), field('intro', 'Introduction', 'Browse selected work across our three core disciplines.', '浏览我们三个核心领域的精选作品。', 'textarea'), ...categoryFields.map(item => ({ ...item, en: item.key.includes('Button') ? 'Explore Category' : item.key.includes('Link') ? item.en.replace('services/','work/') : item.en, zh: item.key.includes('Button') ? '浏览类别' : item.key.includes('Link') ? item.zh.replace('services/','work/') : item.zh }))
    ]},
